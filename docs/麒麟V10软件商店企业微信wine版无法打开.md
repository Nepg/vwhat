...
```bash
exagear -- /opt/cxoffice21/bin/wine --bottle wecom "/home/用户名/桌面/WeCom_5.0.9.6063.exe"
```
...
```bash
exagear -- /opt/cxoffice21/bin/wine --bottle wecom winecfg
```
...
| 事项         | 详细说明                                                                       |
| ------------ | ------------------------------------------------------------------------------ |
| 容器名称固定 | ARM 仅支持 `wecom`，容器数据路径 `~/.cxoffice21/wecom`，不可自定义修改         |
| 强制启动命令 | ARM 必须使用 `/opt/cxoffice21/bin/wine --bottle wecom`，调用系统 wine 无法运行 |
| 首次启动性能 | ARM 首次打开较慢（ExaGear 生成指令缓存），二次及后续启动速度恢复正常           |
| 文件版本匹配 | X86 替换的 `WXWork.exe` 需和安装包版本完全一致                                 |
